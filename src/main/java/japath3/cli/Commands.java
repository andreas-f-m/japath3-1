...
//				.numberOfArgs(2)
	...
	private static void checkSchema(String oSchema, String oType, Node n) {
		if (oSchema != null) {
			if (oType.equals("xml")) throw new JapathException("no schema for 'xml' allowed");
			String viol = null;
			try {
				viol = Schema.checkValidity(n, oSchema);
			} catch (JapathException e) {
				throw new JapathException("--constraints/schema: " + e.getMessage());
			}
			if (viol != null) throw new JapathException(Kind.ConstraintViolation, viol);
		}
	}
}
