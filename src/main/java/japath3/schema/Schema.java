	...
	public static class SchemaBoolExpr extends BoolExpr {
		...
		public NodeIter eval(Node node) {
			...
			case not:
				b = true;
				for (Expr e : exprs) {
					boolean b_ = testIt(e.eval(node));
					// if (ctx.checkValidity() && (op == not ? b : !b))
					b_ = op == not ? !b_ : b_; 
					if (ctx.checkValidity() && !b_) schema.addViolation(AndOp, e, node);
					b = b_ && b;
				}
				break;
			case or:
...
//			return singleBool(op == not ? !b : b, node);
	...
	public Schema genCompleteness(boolean genCompleteness) {
		this.genCompleteness = genCompleteness;
		return this;
	}
...
//			throw new JapathException("schema top expression has to be 'and(...)'");
	...
	public static String checkValidity(Node n, String schemaText) {
		...
		return schema.setSchema(schemaText).checkValidity(n) ? null : schema.annotatedViolations(n);
	}
		...
		Japath.walkr(n, (Node x, Kind kind, int level, int orderNo, boolean isLast) -> {
			if (kind == Kind.Pre) {
				if (propHits.nonEmpty() && x.isStruct()) {
					...
					if (diff.nonEmpty()) {
						ok.r = false;
						addViolation(ViolationKind.Completeness,
								"!!! additional selectors [" + diff.mkString(", ") + "] not covered by schema",
								x);
					}
				}
			}
