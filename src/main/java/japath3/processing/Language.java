...
public class Language {
	...
	public static class Env {
		io.vavr.collection.Map<String, ParamExprDef> defs = HashMap.empty();
	}
...
//		p = parse(stringify(p));
	...
	private static PathExpr parse(Env env, String path, boolean schemaProc) {
...
//	private static PegjsEngineRhino pegjsEngine;
	
	private static int cacheMax = 500;
...
//		}
	...
	private static List<Expr> buildExpr(Env env, final Node ast, boolean schemaProc) {
		...
		if ((x = ast.get("def").node()) != nil) {
			...
			ParamExprDef ped = paramExprDef(name, buildExpr(env, x.node("expr"), schemaProc).get(0));
		...
		if ((x = ast.get("union").node()) != nil) return List.of(union(x.val("arrayFlag"), subExprs(env, schemaProc, x)));
...
//		else return List.empty();
	...
	private static String stringify0(Expr e) {
		...
		if (e instanceof Bind) {
			...
			return collect(e, ".");
		} else if  (e instanceof ParamExprDef) {
			return "def(" + ((ParamExprDef) e).name + ", " + stringify0(((ParamExprDef) e).exprs[0]) + ")";
		} else if  (e instanceof ParamExprAppl) {
			return ((ParamExprAppl) e).name + "(" + collect(((ParamExprAppl) e).params, ", ") + ")";
...
//			return "`" + ((Property) e).name.replace("`", "\\`") + "`";
			...
			return ((Union) e).arrayFlag ?   "[" + collect(e, ", ") + "]" : "union(" + collect(e, ", ") + ")";
